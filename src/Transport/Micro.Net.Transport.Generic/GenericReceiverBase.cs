...
namespace Micro.Net.Transport.Generic
{
    public abstract class GenericReceiverBase : IReceiver
    {
        ...
        protected async Task Dispatch<TRequest, TResponse>(IReceiveContext<TRequest,TResponse> context)
        {
            await _pipeChannel.Handle<IReceiveContext<TRequest, TResponse>>(context);
        }
    }
}
